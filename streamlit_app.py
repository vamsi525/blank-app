...
if source_file and target_file:
    ...
    if st.button("Generate XSLT"):
        ...
        if response.status_code == 200:
            ...
            st.error(
                f"Failed to generate XSLT: Status Code {response.status_code}, Error: {response.json() if response.headers.get('Content-Type') == 'application/json' else response.text}"
            )
